...
// ---- AI targets & movement ----
target_obj       = obj_Player1;   // <— change to your player object
root_move_speed  = 2.0;
root_turn_rate   = 2.0;

// ---- default fire timers per-part (used by hands/head) ----
default_hand_fire_cd = 18;   // 0.6s @ 30fps
default_head_fire_cd = 24;

// cache layer id for bullets (optional: make an "Enemies" layer)
bullet_layer = layer; // you can swap to "Enemies" if you have that layer
...
if (kaiju_mode == "mech") {
    var bp = [
        ...
        ["head",   KaijuRole.HEAD,       sprMech_Head,    1200, 2, "torsoU",   -40 * scale,  -350 * scale,  0,  true,  true,-3 ],
        ["torsoU", KaijuRole.TORSO_U,    sprMech_TorsoU,  1800, 3, "core",   0,  -80 * scale,  0,  true,  false,-2],
        ["torsoL", KaijuRole.TORSO_L,    sprMech_TorsoL,  2000, 5, "core",   0,   80 * scale,  0,  true,  false,-1],
		
        ["armU_L", KaijuRole.ARM_U_L,    sprMech_ArmU,    1100, 2, "torsoU",-152 * scale,  -150 * scale, 0,  true,  true,-4 ],
		
	   ["armL_L", KaijuRole.ARM_L_L,    sprMech_ArmL,     900, 1, "armU_L",250 * scale,   8 * scale, 0,  true,  true,-5],
       ["handL",  KaijuRole.HAND_L,     sprMech_Hand,     700, 1, "armL_L",250 * scale,  12 * scale, 0,  true,  true,-6 ],
        
		["armU_R", KaijuRole.ARM_U_R,    sprMech_ArmU,    1100, 2, "torsoU", 0,  -180 * scale,  0,  true,  true, -1],
        ["armL_R", KaijuRole.ARM_L_R,    sprMech_ArmL,     900, 1, "armU_R", 250 * scale,   8 * scale,0,  true,  true, -2],
        ["handR",  KaijuRole.HAND_R,     sprMech_Hand,     700, 1, "armL_R", 250 * scale,  12 * scale,0,  true,  true, -3],
       
	   
	 
	   ["thighL", KaijuRole.THIGH_L,    sprMech_Thigh,   1300, 2, "torsoL",-150 * scale,  150 * scale,  270,  true,  true, -6],
        ["calfL",  KaijuRole.CALF_L,     sprMech_Calf,    1100, 2, "thighL",  250 * scale,  0,  0,  true,  true, -5 ],
        ["footL",  KaijuRole.FOOT_L,     sprMech_Foot,     900, 1, "calfL",   150 * scale,  0,  0,  false,  true,  -4],
         
	   ["thighR", KaijuRole.THIGH_R,    sprMech_Thigh,   1300, 2, "torsoL", 100 * scale,  130 * scale,  270,  true,  true, 1],
        ["calfR",  KaijuRole.CALF_R,     sprMech_Calf,    1100, 2, "thighR",  250 * scale,  20 * scale,  0,  true,  true, 2],
        ["footR",  KaijuRole.FOOT_R,     sprMech_Foot,     900, 1, "calfR",   150 * scale,  16 * scale,  0,  false,  true, 3],
       /*
    ...
    for (var i = 0; i < array_length(bp); i++) {
        ...
        with (inst) {
            ...
            core_root = root_id;
			 ...
			 fire_cd = 0;        // cooldown counter, counts down in Step
		    fire_cd_max = 0;    // how long to wait between shots

		    // assign fire rates by role
		    if (role == KaijuRole.HAND_L || role == KaijuRole.HAND_R) {
		        fire_cd_max = other.default_hand_fire_cd;
		    } else if (role == KaijuRole.HEAD) {
		        fire_cd_max = other.default_head_fire_cd;
		    }
