/// objHumanPhys.Create  — single-actor physics human with jetpack + arms
// --- tuning ---
hp = 100;

// movement
move_accel_gnd   = 24000;  // horizontal force on ground
move_accel_air   = 12000;  // in-air strafe
max_run_speed    = 6.5;    // px/step in physics world units
jump_impulse     = 12000;  // instantaneous upward impulse

// jetpack
jet_thrust_up    = 22000;  // continuous up force while held
jet_thrust_side  = 9000;   // side assist while jetting
fuel_max         = 100;
fuel             = fuel_max;
fuel_burn_rate   = 40;     // per step while jetting
fuel_recharge    = 22;     // per step on ground
jet_on           = false;
...
fire_cd          = 0;
bullet_speed     = 14;
recoil_impulse   = 700;    // tiny kickback
...
elbow_len        = 12;
hand_len         = 14;
back_arm_offset  = -6;     // z-sort hack: push back arm slightly

hip_ofs          = {x: 0, y: 10};
leg_len          = 16;
foot_len         = 6;
walk_cycle       = 0;      // advance when moving

// sprites (replace with your assets)
spr_body         = sprHuman_Body;
spr_head         = sprHuman_Head;
spr_jetpack      = sprSpraycan_Jetpack;
spr_gun          = sprRayGun;    // or your gun
spr_arm          = sprHuman_Arm_Idle;      // simple bar/arm sprite (or draw_line if you prefer)
spr_hand         = sprHuman_Arm_Hand;
spr_leg          = sprHuman_Leg_Walk;
spr_foot         = sprHuman_Pants_Walk_Feet;
...
physics_fixture_set_linear_damping(fx, 1.6);
physics_fixture_set_angular_damping(fx, 1000);
physics_fixture_set_awake(fx, true);
physics_fixture_bind(fx, id);
...
// input cache
_left  = 0; _right = 0; _jump = 0; _jet = 0;
